...
// Per-OS data directory (sibling of the persistent browser profile). Namespaced
// to this skill so the launched browser, its profile, and its port file are
// independent of any other app's live-browser instance (e.g. a host desktop app).
export function dataDir() {
  if (process.platform === 'win32') {
    return resolve(process.env.LOCALAPPDATA || resolve(homedir(), 'AppData/Local'), 'claude-live-browser');
  }
  if (process.platform === 'darwin') {
    return resolve(homedir(), 'Library/Application Support/claude-live-browser');
  }
  return resolve(process.env.XDG_CACHE_HOME || resolve(homedir(), '.cache'), 'claude-live-browser');
}
